...
import { ArrowLeft, Trophy, Flame, Play } from 'lucide-react';
import { motion } from 'motion/react';
import { sound } from '../../utils/sound.js';

export const HighScoresScreen = ({ highScores, onNavigate }) => {
  return (
    <div id="high-scores-screen" className="relative w-full h-full flex flex-col items-center justify-between p-6 sm:p-10 z-30 select-none overflow-y-auto">
          ...
          return (
            ...
            >
              <div className="w-12 flex items-center">
                {isTop3 ? (
                  <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${rankColors[entry.rank - 1]} font-arcade text-sm flex items-center justify-center font-bold shadow-md border`}>
                    {entry.rank}
                  </div>
                ) : (
                  <span className="font-arcade text-base text-slate-500 pl-2">{entry.rank}</span>
                )}
              </div>

              <div className="flex-1 flex items-center gap-2">
                <span className="font-arcade text-base sm:text-lg text-white tracking-wider">{entry.playerName}</span>
                {entry.rank === 1 && <Trophy className="w-4 h-4 text-amber-400 fill-amber-400 shrink-0" />}
              </div>
              ...
              <div className="w-24 text-right">
                <span className={`font-arcade text-xl sm:text-2xl tracking-wide ${entry.rank === 1 ? 'text-amber-300' : 'text-slate-200'}`}>
