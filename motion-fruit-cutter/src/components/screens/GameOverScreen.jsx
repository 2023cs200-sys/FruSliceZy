...
export const GameOverScreen = ({ stats, onPlayAgain, onNavigate }) => {
  ...
  useEffect(() => {
    if (isNewHighScore) {
      ...
      try {
        confetti({ particleCount: 100, spread: 80, origin: { y: 0.6 }, colors: ['#f59e0b', '#ef4444', '#10b981', '#3b82f6', '#f43f5e'] });
        setTimeout(() => {
          confetti({ particleCount: 60, angle: 60, spread: 55, origin: { x: 0 }, colors: ['#fbbf24', '#f97316', '#ffffff'] });
          confetti({ particleCount: 60, angle: 120, spread: 55, origin: { x: 1 }, colors: ['#fbbf24', '#f97316', '#ffffff'] });
        }, 300);
      } catch {
        // Safe fallback
      }
  ...
  return (
    <div id="game-over-screen" className="relative w-full h-full flex flex-col items-center justify-between p-6 sm:p-10 z-30 select-none overflow-y-auto">
      <div className="w-full max-w-xl my-auto flex flex-col items-center">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center p-5 rounded-2xl bg-[#0c0d12]/80 border border-white/10 z-10">
            <span className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">FINAL SCORE</span>
            <div className="text-5xl sm:text-6xl font-black text-[#ffcc00] drop-shadow-[0_0_15px_rgba(255,204,0,0.5)] tracking-tight leading-none" style={{ fontFamily: 'Arial, sans-serif' }}>
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 z-10">
            <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-[#1c1f2e]/80 border border-white/10">
              ...
              <div>
                <span className="block text-[11px] font-bold text-gray-400 uppercase">BEST SCORE</span>
                <span className="font-mono font-bold text-lg sm:text-xl text-[#ffcc00]">{stats.bestScore.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-[#1c1f2e]/80 border border-white/10">
              ...
              <div>
                <span className="block text-[11px] font-bold text-gray-400 uppercase">FRUITS CUT</span>
                <span className="font-mono font-bold text-lg sm:text-xl text-emerald-400">{stats.fruitsCut}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-[#1c1f2e]/80 border border-white/10">
              ...
              <div>
                <span className="block text-[11px] font-bold text-gray-400 uppercase">MAX COMBO</span>
                <span className="font-mono font-bold text-lg sm:text-xl text-orange-400">x{stats.maxCombo}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-[#1c1f2e]/80 border border-white/10">
              ...
              <div>
                <span className="block text-[11px] font-bold text-gray-400 uppercase">BOMBS HIT</span>
                <span className="font-mono font-bold text-lg sm:text-xl text-rose-400">{stats.bombsHit}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2 z-10">
            <button
              ...
              onClick={() => { sound.playButton(); sound.playSlice(2); onPlayAgain(); }}
