...
import { ArrowLeft, Volume2, Music, Sliders, Activity, Vibrate, Sparkles, Sword } from 'lucide-react';
import { GameSettings, ScreenState } from '../../types';
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-2xl flex items-center justify-between">
        ...
        <h1 className="font-arcade text-3xl sm:text-5xl text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-fuchsia-300 to-amber-300 arcade-text-shadow">
          SETTINGS
        </h1>
      ...
      <div className="w-full max-w-2xl my-auto bg-slate-900/90 border-2 border-purple-500/40 rounded-3xl p-6 sm:p-8 backdrop-blur-md shadow-2xl flex flex-col gap-6">
        ...
        <div className="space-y-3">
          <span className="text-xs font-bold text-purple-400/80 uppercase tracking-widest block">
            Audio & Feedback
          </span>
          ...
          <div className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-800/60 border border-slate-700/60">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="font-arcade text-base text-white tracking-wider block">
                  SOUND EFFECTS
                </span>
            ...
            <button
              ...
              className={`w-14 h-8 rounded-full p-1 transition-colors cursor-pointer ${
                settings.soundEffects ? 'bg-purple-600' : 'bg-slate-700'
              }`}
            >
              <div
          ...
          <div className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-800/60 border border-slate-700/60">
            ...
            <button
              onClick={toggleMusic}
              className={`w-14 h-8 rounded-full p-1 transition-colors cursor-pointer ${
                settings.music ? 'bg-purple-600' : 'bg-slate-700'
              }`}
            >
              <div
                className={`w-6 h-6 rounded-full bg-white transition-transform ${
          ...
          <div className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-800/60 border border-slate-700/60">
            ...
            <button
              ...
              className={`w-14 h-8 rounded-full p-1 transition-colors cursor-pointer ${
                settings.vibration ? 'bg-purple-600' : 'bg-slate-700'
              }`}
            >
              <div
                className={`w-6 h-6 rounded-full bg-white transition-transform ${
        ...
        <div className="space-y-4">
          <span className="text-xs font-bold text-cyan-400/80 uppercase tracking-widest block">
            Motion Sensors (WebSocket)
          </span>
          ...
          <div className="p-4 rounded-2xl bg-slate-800/60 border border-slate-700/60 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-sm font-bold text-cyan-300">
                {settings.controllerSensitivity} / 10
              </span>
            </div>
            <input
              type="range"
              ...
              onChange={(e) => onUpdateSettings({ controllerSensitivity: Number(e.target.value) })}
          ...
          <div className="p-4 rounded-2xl bg-slate-800/60 border border-slate-700/60 flex flex-col gap-2">
            ...
            <input
              ...
              onChange={(e) => onUpdateSettings({ motionSmoothing: Number(e.target.value) })}
              className="w-full accent-cyan-400 cursor-pointer"
            />
          </div>
        </div>

        {/* BLADE SKINS & GRAPHICS */}
        <div className="space-y-3">
          <span className="text-xs font-bold text-amber-400/80 uppercase tracking-widest block">
            Blade Trail Style & Quality
          </span>

          {/* Blade selector */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            {(['CRIMSON', 'CYBER_AZURE', 'EMERALD', 'GOLDEN'] as const).map((style) => {
              const names = {
                CRIMSON: 'Crimson',
                CYBER_AZURE: 'Azure',
                EMERALD: 'Jade',
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    sound.playButton();
                    onUpdateSettings({ bladeStyle: style });
                  }}
                  className={`py-2 px-3 rounded-xl border text-xs font-arcade tracking-wider transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                    isSelected
                      ? `${colors[style]} ring-2 ring-white/50 scale-105 shadow-md`
          ...
          {/* Graphics Quality */}
          <div className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-800/60 border border-slate-700/60">
            <span className="font-arcade text-sm text-white tracking-wider">
              GRAPHICS QUALITY
            </span>
